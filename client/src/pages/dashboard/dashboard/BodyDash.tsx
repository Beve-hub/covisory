...
const StatCard = ({ color, label, amount, percentage }:ChartProps) => (
  <div className='bg-[var(--card-color)] items-center shadow-sm p-2 '>
    <p className='flex items-center text-sm'>
      ...
      <span className='text-[var(--gray-color)] text-xs ml-1'>{label}</span>
    </p>
    <h2 className='font-bold text-sm'>{amount}</h2>
    <p className='flex items-center text-sm font-semibold'>
      <IoMdArrowDropup color='#016119' />
      <span className='text-[var(--success-color)] text-xs  ml-1 '>{percentage}</span>
    </p>
  </div>
);

const Card = () => (
  <div className='rounded-lg bg-[var(--card-color)] items-center p-6 w-full h-[26rem] '>
    <h2 className='text-md font-bold  pb-6'>Investment Categories</h2>
  <div className='flex flex-col items-center justify-center md:flex-row gap-6 '>
  <div className='relative max-w-[40rem] h-[15rem] mx-auto'>
